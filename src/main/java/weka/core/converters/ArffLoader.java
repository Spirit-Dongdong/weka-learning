...
public class ArffLoader
  ...
  public static class ArffReader
    ...
    protected void readHeader(int capacity) throws IOException {
      ...
      if (Instances.ARFF_RELATION.equalsIgnoreCase(m_Tokenizer.sval)) {//读到@relation，后面是实体名（如cpu）
        getNextToken();
        relationName = m_Tokenizer.sval;//sval是string value的简写
        getLastToken(false);//验证作用
      } else {
        errorMessage("keyword " + Instances.ARFF_RELATION + " expected");
      }

      // Create vectors to hold information temporarily.
      FastVector attributes = new FastVector();//获取属性，属性定义是这样的：   @attribute MYCT real
      ...
      while (Attribute.ARFF_ATTRIBUTE.equalsIgnoreCase(m_Tokenizer.sval)) {//如果读取到的是@attribute，则进入解析属性的步骤
        attributes = parseAttribute(attributes);
      }

      // Check if data part follows. We can't easily check for EOL.
      if (!Instances.ARFF_DATA.equalsIgnoreCase(m_Tokenizer.sval)) {//属性结束之后有个@data，这里检查一下
    ...
    protected FastVector parseAttribute(FastVector attributes) throws IOException {
      ...
      FastVector attributeValues;//属性值，用vector...

      //要解析的属性是这样的： MYCT real
      // Get attribute name.
      getNextToken();//空格
      attributeName = m_Tokenizer.sval;//属性名称，就是上面的MYCT
      getNextToken();//空格

      // Check if attribute is nominal.这里开始解析属性类型了，没什么说的。。。
      if (m_Tokenizer.ttype == StreamTokenizer.TT_WORD) {
        ...
        if (m_Tokenizer.sval.equalsIgnoreCase(Attribute.ARFF_ATTRIBUTE_REAL) ||
          ...
          readTillEOL();
        } else if (m_Tokenizer.sval.equalsIgnoreCase(Attribute.ARFF_ATTRIBUTE_STRING)) {//字符串
          attributes.
            addElement(new Attribute(attributeName, (FastVector)null,
                attributes.size()));
          readTillEOL();
        } else if (m_Tokenizer.sval.equalsIgnoreCase(Attribute.ARFF_ATTRIBUTE_DATE)) {//日期
          String format = null;
          if (m_Tokenizer.nextToken() != StreamTokenizer.TT_EOL) {
            if ((m_Tokenizer.ttype != StreamTokenizer.TT_WORD) &&
                (m_Tokenizer.ttype != '\'') &&
        ...
        } else if (m_Tokenizer.sval.equalsIgnoreCase(Attribute.ARFF_ATTRIBUTE_RELATIONAL)) {//关系形式
          readTillEOL();
          // Read attributes for subrelation
          // First, save current set of attributes下面这些是针对较复杂属性的(不知道这个什么时候会被用到。。)
