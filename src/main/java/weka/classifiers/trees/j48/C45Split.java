...
public class C45Split extends ClassifierSplitModel {
	...
	/** Desired number of branches. 分支个数，对于非名字型属性（Nominal）都是2 */
	private int m_complexityIndex;

	/** Attribute to split on. 本次分割的属性index */
	private int m_attIndex;

	/** Minimum number of objects in a split. */
	private int m_minNoObj;

	/** Value of split point. 以（属性）的值作为分割点 */
	private double m_splitPoint;

	/** InfoGain of split. 信息增益 */
	private double m_infoGain;

	/** GainRatio of split. 信息增益率，C4.5选择分隔属性的依据 */
	...
	private void handleNumericAttribute(Instances trainInstances)
		...
		double defaultEnt;
		double minSplit;// 每个分隔中必须保证的实例个数
		Instance instance;
		int i;

		// Current attribute is a numeric attribute.对数值型的，分成2个bag
		...
		// 如果firstMiss<=2*minSplit,说明已经不够分了（要被分成2部分，至少有一部分的个数小于minSplit）
		if (Utils.sm((double) firstMiss, 2 * minSplit))
			return;

		// Compute values of criteria for all possible split
		// indices.
		// 计算一个属性下的每个可能分割点，此时instance已经由该属性下的值小到大排好序，
		// 如【1，3，6，7，9】，就是要计算是在2分割，还是4.5分割，还是6.5...
		defaultEnt = infoGainCrit.oldEnt(m_distribution);
		while (next < firstMiss) {

			if (trainInstances.instance(next - 1).value(m_attIndex) + 1e-5 < trainInstances
					.instance(next).value(m_attIndex)) { // 之前已经对instance排好序了，所以这里应该是区分相等/相差值过小的情况
